...
### Phased build order
- **P7a — done:** the pure payload-building/parsing `core/` functions, the `BackendService` autoload, and the `LeaderboardView` panel (list + Link Account form) — all built and unit-tested without a live server.
- **P7b — done, live-verified end to end:** the user's real Supabase project (Anonymous Sign-Ins enabled, schema SQL run) is wired in and confirmed working for real, on-device, against the live project:
  - A real anonymous sign-in from the device (not curl) created a real session.
  - The device's real hunter synced and appeared correctly on the live global leaderboard alongside a curl-created test row, matching the on-device save exactly (rank/level/power/floor).
  - The account-linking endpoint was verified live via curl (a fresh anonymous session token) rather than risking the device's own persistent identity: a fake `@example.com` address was correctly rejected server-side (`email_address_invalid`), and a real address correctly passed validation and reached Supabase's built-in email sender, which then hit its (low, dev-tier) rate limit (`over_email_send_rate_limit`) — confirming the endpoint and payload shape are correct without needing to complete a real email confirmation.
  - **Real gap found along the way, fixed:** the Android export (`export_presets.cfg`) never had the `INTERNET` permission enabled for this project's custom build — nothing before this patch needed real network access, so it went unnoticed. Every earlier native-plugin permission (location, health) was explicit; this one wasn't, and Godot's custom-build export only includes what's explicitly listed. Fixed by adding `permissions/internet=true`; confirmed via `adb shell dumpsys package` before and after.
  - **Real gap found, not yet fixed:** Supabase's built-in email sender is dev-tier and rate-limited (hit after a couple of test sends) — fine for testing, but real production account-linking will need a custom SMTP provider configured in the Supabase dashboard before this ships to real users. Flagged, not built.
  - One test leaderboard row (`CurlTestHunter`, from curl testing) is still live on the real board — no `delete` RLS policy exists yet (only select/insert/update were in the schema), so it can't be cleared via the app; removable from the Supabase Table Editor directly if wanted.
- **P7c — later patches:** friends scope, regional scope (with its own explicit §27-style privacy note), real server-side anti-cheat beyond the CHECK-constraint floor, custom SMTP for account-linking emails.
