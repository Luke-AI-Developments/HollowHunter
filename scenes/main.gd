...
@onready var shadow_reveal_card: ShadowRevealCard = $GameUI/ShadowRevealPanel
...
func _setup_gear_panels() -> void:
	if not hunter_gear_button.pressed.is_connected(_on_hunter_gear_button_pressed):
		...
		shadow_reveal_card.state_changed.connect(_on_state_changed)
		shadow_reveal_card.manage_gear_requested.connect(_on_manage_gear_requested)
...
func _on_battle_finished(won: bool) -> void:
	...
	if won:
		...
		if GameLogic.attempt_claim(gate.get("monster_extract_chance", 0.0), state.level, rng):
			...
			_pending_reveal_shadow_id = claimed["instance_id"]
...
## CLAIM arms _pending_reveal_shadow_id, so this is a no-op otherwise
## (level-ups, rewards, rank trials, etc.). Onboarding / scripted-first-gate
## claims never route through _on_battle_finished, so they're excluded
## structurally.
func _on_system_panel_dismissed() -> void:
	if _pending_reveal_shadow_id == "":
		return
	var id := _pending_reveal_shadow_id
	_pending_reveal_shadow_id = ""
	shadow_reveal_card.show_for(id, ShadowRevealCard.Mode.CLAIM)


func _on_manage_gear_requested(instance_id: String) -> void:
	var idx := state.army.find_custom(
		func(s: Dictionary) -> bool: return s.get("instance_id", "") == instance_id
	)
	if idx < 0:
		return
	shadow_gear_view.open()
	shadow_gear_view.jump_to_index(idx)
